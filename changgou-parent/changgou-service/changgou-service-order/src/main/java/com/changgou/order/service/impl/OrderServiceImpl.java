...
public class OrderServiceImpl implements OrderService {
    ...
    public void add(Order order){
        ...
        for (OrderItem orderItem : values) {
            if (order.getSkuIds().contains(orderItem.getSkuId())){
                ...
                //金额，先获取skuid，再用skuid查找最新价格
                Result<Sku> skuResult = skuFeign.findById(orderItem.getSkuId());
                totalMoney += skuResult.getData().getPrice();//金额
                //2.添加订单选项表的数据
        ...
        System.out.println("发出消息的时间" + simpleDateFormat.format(new Date()));
        //添加订单，延时还没付款就回滚
        //路由名字（消息发给谁）， 消息内容, 队列的相关信息这里设置一个延时
    ...
    public void deleteOrder(String outtradeno) {
        Order order = (Order) orderMapper.selectByPrimaryKey(outtradeno);
        order.setUpdateTime(new Date());
