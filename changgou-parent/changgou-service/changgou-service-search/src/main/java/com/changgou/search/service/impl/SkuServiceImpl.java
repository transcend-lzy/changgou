...
//        }
        ...
        //防止出现规格相同的情况出现
        //规则 要求 字段 是一个keyword类型的  spec.keyword，keyword类型的数据不分词
        ...
        nativeSearchQueryBuilder.withHighlightFields(new HighlightBuilder.Field("name"));  //指定高亮域，这里将商品名称设置为高亮
        //设置preTags前缀 和 postTags后缀  fragmentSize 是关键词数据长度
        ...
        //从多个字段中搜索数据，关键词筛选
        if (!StringUtils.isEmpty(keywords)) nativeSearchQueryBuilder.withQuery(QueryBuilders.multiMatchQuery(keywords,"name","categoryName","brandName"));



        //========================过滤查询 开始=====================================
        //不同的条件之间是有过滤关系的（比如某一个华为品牌下不应该有户外工具的分类）
        // 所以构建布尔查询，并将各个条件组装进去，并构建互相筛选的条件
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();

        //4.4 过滤查询的条件设置   商品分类的条件
        String category = searchMap.get("category");

        //如果用户已经点击了分类了，这时候就不应该展示分类的数据了，这时category不为空，也不应该展示category的数据
...
//如果key前面带有spec_前缀，则表示这是一个规格条件
        ...
        String price = searchMap.get("price");// 0-500  3000-*   价格区间是自己设置的
        ...
        //第二个参数:指定当前的页的显示的行
        String pageNum1 = "1";  //不设置分页信息就默认是第一页
        if(searchMap.get("pageNum") != null) pageNum1 = searchMap.get("pageNum");
        Integer pageNum=Integer.valueOf(pageNum1);

        Integer pageSize=30;//默认查询记录数30
        ...
        if(!StringUtils.isEmpty(sortField) && !StringUtils.isEmpty(sortRule)) {
            //执行排序
            nativeSearchQueryBuilder.withSort(SortBuilders.fieldSort(sortField)  //指定排序域
                    .order(sortRule.equalsIgnoreCase("ASC")? org.elasticsearch.search.sort.SortOrder.ASC: SortOrder.DESC)  //指定规则
                    );
        ...
        //6.执行查询
        AggregatedPage<SkuInfo> skuInfos = elasticsearchTemplate.queryForPage(nativeSearchQuery,  //搜索条件封装
                SkuInfo.class,   //数据集合要转换的类型的字节码文件
                new SearchResultMapperImpl()   //执行搜索后，将数据结果集封装到该对象中
        );


        // 6.2 获取聚合分组结果  获取商品分类的列表数据
        //StringTerms表示把查询到的集合数据转换为string类型的集合
        //.getAggregation是获取根据categoryName字段进行分组的集合，里面参数是skuCategorygroup因为前面起了别名
    ...
    private Map<String, Set<String>> getStringSetMap(StringTerms stringTermsSpec) {
        ...
        if (stringTermsSpec != null) {
            ...
            for (StringTerms.Bucket bucket : stringTermsSpec.getBuckets()) {
                //2.取出结果集的每一行数据()   {"手机屏幕尺寸":"5.5寸","网络":"电信4G","颜色":"白","测试":"s11","机身内存":"128G","存储":"16G","像素":"300万像素"}
